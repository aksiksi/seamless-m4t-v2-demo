...
audio_array_from_text = model.generate(**text_inputs, tgt_lang="arb", return_intermediate_token_ids=True)[0].cpu().numpy().squeeze()

# from audio
audio, orig_freq =  torchaudio.load("https://www2.cs.uic.edu/~i101/SoundFiles/preamble10.wav")
audio =  torchaudio.functional.resample(audio, orig_freq=orig_freq, new_freq=16_000) # must be a 16 kHz waveform array
audio_inputs = processor(audios=audio, return_tensors="pt")
audio_array_from_audio = model.generate(**audio_inputs, tgt_lang="arb")[0].cpu().numpy().squeeze()
...
# text from audio
output_tokens = model.generate(**audio_inputs, tgt_lang="arb", generate_speech=False)
